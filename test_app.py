...
class CapstoneTestCase(unittest.TestCase):
	...
	def setUp(self):
		...
		self.database_path = "postgres://txdjjhlbnduuhp:2d1346031b75afc27357c0cba50053231c6cb1c81606f27b0c64dc30d93702ce@ec2-52-202-66-191.compute-1.amazonaws.com:5432/dbhh4ikpsi78ag"
	...
	def post_actor(self):
	  ...
	  res = self.client.post('/actors', \
		json={'name': 'Samuel L Jackson', 'age': 66, 'gender':'Male'}, headers = auth_header)
	  data = json.loads(res.data)
	...
	def post_actor_failed(self):
	  ...
	  res = self.client.post('/actors', \
		json={'name': True, 'age': 66, 'gender':'Male'}, headers = auth_header)
	  data = json.loads(res.data)
	...
	def post_movie(self):
	  ...
	  res = self.client.post('/movies', \
		json={'title': 'Silance of the Lambs', 'release_date': '1991'}\
		  , headers = auth_header)
	  data = json.loads(res.data)
	  self.assertTrue(data['success'])
	  self.assertEqual(res.status_code, 200)
	def post_movie_failed(self):
	  ...
	  res = self.client.post('/movies', \
		json={'title': True, 'release_date': 1991}\
		  , headers = auth_header)
	  data = json.loads(res.data)
	  self.assertEqual(res.status_code, 422)
	  self.assertFalse(data['success'])
	...
	def edit_movie(self):
	  ...
	  res = self.client.patch('movies/1',\
		json={'title': 'The Departed', 'release_date': '2006'},\
		headers = auth_header)
	  data = json.loads(res.data)
	  self.assertTrue(data['success'])
	  self.assertEqual(res.status_code, 200)
	def edit_movie_failed(self):
	  res = self.client.patch('movies/1982', json={'title': ' Departed', 'release_date': '2006'})
	  data = json.loads(res.data)
	  self.assertEqual(res.status_code, 404)
	  self.assertFalse(data['success'])
	...
	def edit_actor(self):
	  ...
	  res = self.client.patch('actors/1',\
		json={'name': 'Leonardo Dicaprio', 'age':'43', 'gender':'Male'},\
		headers = auth_header)
	  data = json.loads(res.data)
	  self.assertTrue(data['success'])
	  self.assertEqual(res.status_code, 200)
	def edit_actor_failed(self):
	  res = self.client.patch('actors/1982', \
		json={'name': 'Leonardo Dicaprio', 'age':'43', 'gender':'Male'})
	  data = json.loads(res.data)
	  self.assertEqual(res.status_code, 404)
	  self.assertFalse(data['success'])
